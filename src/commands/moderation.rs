...
pub async fn kick(
    ...
    let guild = ctx.guild_id().ok_or("Must be in a guild")?;
    let reason_str = reason.as_deref().unwrap_or("No reason provided");

    guild.kick_with_reason(ctx.http(), user.id, reason_str).await?;
...
pub async fn timeout(
    ...
    let until = serenity::Timestamp::from(
...
pub async fn ban(
    ...
    guild.ban_with_reason(ctx.http(), user.id, days, reason_str).await?;
...
#[poise::command(slash_command, prefix_command, required_permissions = "MANAGE_MESSAGES")]
pub async fn warn(
    ctx: Context<'_>,
    #[description = "The user to warn"] user: serenity::User,
    #[description = "The reason for the warning"] reason: String,
) -> Result<(), Error> {
    // TODO: persist warning to database
    ctx.send(poise::CreateReply::default().embed(
        serenity::CreateEmbed::default()
...
pub async fn purge(
    ...
    let reply = ctx
        .say(format!("🗑 Deleted **{}** message(s).", ids.len()))
        .await?;
